...
/**
 * Address Parser Implementation that splits the input address based on space
 * delimiter and then extracts the street name and street number.
 */
public class PlainAddressParserImpl implements AddressProviderParser {
    public String[] parse(String address) throws InvalidInputException {
        ...
        for (int i = 0; i < words.length; i++) {
            if (AddressProviderUtil.hasDigits(words[i])) {
